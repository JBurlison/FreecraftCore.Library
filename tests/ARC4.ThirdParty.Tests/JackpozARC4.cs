namespace ARC4.ThirdParty
{
	public class JackpozARC4
	{
		private readonly byte[] state;
		private byte x, y;

		public JackpozARC4(byte[] key)
		{
			state = new byte[256];
			x = y = 0;
			KeySetup(key);
		...
		public int Process(byte[] buffer, byte[] outputBuffer, int start, int count)
		{
			return InternalTransformBlock(buffer, start, count, outputBuffer, start);
		}
